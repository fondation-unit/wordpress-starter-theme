...
class GenerateFacets
{
    ...
    public function createFacetsArray($response): array
    {
        ...
        foreach ($response as $resp) {
            ...
            foreach ($resp->types_pedagogiques as $type) {
                if (! array_key_exists($type, $this->typesArray)) {
                    $this->typesArray[$type]['count'] = 1;
                } else {
                    $this->typesArray[$type]['count'] += 1;
                }
            }
            foreach ($resp->discipline_facet as $facet) {
                $this->cleanDisciplineFacet($facet);
            }
        }
    ...
    private function cleanDisciplineFacet($disciplines): void
    {
        ...
        if (isset($expDisciplines[1])) {
            if (! array_key_exists($expDisciplines[1], $this->disciplinesArray[$expDisciplines[0]])) {
                ...
                $this->disciplinesArray[$expDisciplines[0]][$expDisciplines[1]]['count']++;
            }
        }
    ...
    /**
     * @param $string
     * Echappe les accents pour permettre le tri alpha dans le tableau
     *
     * @return mixed|string
     */
    public function remove_accents($string): string
    {
